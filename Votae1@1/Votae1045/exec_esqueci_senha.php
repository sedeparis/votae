...
<body>
	<div class="container">
	<img src="img/logop.jpg"><br><br>
	<?php
	$user=$_POST['user'];
	$veri=$_POST['verificador'];
	// cria a instrução SQL que vai selecionar os dados dos itens
	$query = ("SELECT * FROM user WHERE email ='$user' AND verificador= '$veri'");
	// executa a query
	...
	if($total == 0) {echo "Usuário não encontrado ou verificador não confere. <a href='indexu.php'>Voltar</a>";}
	if($total > 0) {
	...
	?>
	<form name="form" method="post" action="salva_esqueci_senha.php" onSubmit="return validacao();">
